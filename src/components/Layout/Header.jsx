...
export default function EnhancedHeaderTailwind() {
  ...
  {menuOpen && (
    ...
    >
      {/* Koyu arka plan */}
      <motion.div
        className="absolute inset-0 bg-black/70" // daha koyu arka plan
        onClick={toggleMenu}
        initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}
        exit={{ opacity: 0 }}
      ...
      {/* Menü paneli artık full ekran */}
      <motion.div
  className="absolute inset-0 z-50 flex flex-col h-screen bg-white shadow-2xl border-l border-white/20"
        initial={{ x: "-100%" }}
        animate={{ x: 0 }}
        exit={{ x: "-100%" }}
