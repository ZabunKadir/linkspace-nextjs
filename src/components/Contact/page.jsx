...
const ContactFormFormik = ({ onStatusChange }) => {
  const handleSubmit = async (values, { setSubmitting, resetForm }) => {
    try {
      ...
      onStatusChange({ type: "success", message: "Your message has been sent successfully." });
      resetForm();
    } catch (error) {
      onStatusChange({ type: "error", message: "An error occurred while sending your message. Please try again." });
    } finally {
      setSubmitting(false);
    }
  };
  ...
  return (
    <div className="min-h-screen flex items-center justify-center px-6">
      <div className="flex flex-col items-center justify-center w-full max-w-7xl space-y-6 md:space-y-0 md:flex-row md:space-x-8">
        ...
        >
          <h2 className="text-3xl sm:text-4xl font-bold font-calsans text-black">
            Get in Touch With Us
          </h2>
          <p className="text-lg text-gray-700 mb-6 mt-5 leading-relaxed">
            If you have any questions or feedback, feel free to reach out.
            You can quickly contact us by filling out the form below.
          </p>
        ...
        >
          <h3 className="text-xl font-semibold text-darkPurple mb-4">Contact Form</h3>
